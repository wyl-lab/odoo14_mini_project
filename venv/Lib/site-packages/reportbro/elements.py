...
from .qrcode import qr_image
from .context import Context
from .docelement import DocElementBase, DocElement
from .enums import *
from .errors import Error, ReportBroError
from .rendering import ImageRenderElement, BarCodeRenderElement, QrCodeRenderElement, TableRenderElement, FrameRenderElement, SectionRenderElement
...
class ImageElement(DocElement):
    ...
    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_image(container_offset_x, container_offset_y, self)
...
class BarCodeElement(DocElement):
    ...
    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_barcode(container_offset_x, container_offset_y, self)


class QrCodeElement(DocElement):
    def __init__(self, report, data):
        DocElement.__init__(self, report, data)
        self.content = data.get('content', '')
        self.format = data.get('format', '').lower()
        assert self.format == 'qr'
        self.display_value = bool(data.get('displayValue'))
        self.print_if = data.get('printIf', '')
        self.remove_empty_element = bool(data.get('removeEmptyElement'))
        self.spreadsheet_hide = bool(data.get('spreadsheet_hide'))
    ...
    def prepare(self, ctx, pdf_doc, only_verify):
        ...
        if self.prepared_content:
            try:
                img = qr_image(self.prepared_content, width=self.image_height, version=1, box_size=10, border=4)
            except:
                raise ReportBroError(
                    Error('errorMsgInvalidBarCode', object_id=self.id, field='content'))
            if not only_verify:
    ...
    def get_next_render_element(self, offset_y, container_height, ctx, pdf_doc):
        ...
        if not rv:
            return None, False
        return QrCodeRenderElement(self.report, offset_y, self), True
    ...
    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_qrcode(container_offset_x, container_offset_y, self)
...
class LineElement(DocElement):
    def __init__(self, report, data):
        ...
        self.lineAngle = float(data.get('lineAngle', '0') or '0')
        self.print_if = data.get('printIf', '')
    ...
    def render_pdf(self, container_offset_x, container_offset_y, pdf_doc):
        ...
        x1 = x + self.width*math.cos(math.radians(self.lineAngle))
        y1 = y + self.width * math.sin(math.radians(self.lineAngle))
        pdf_doc.line(x, y, x1, y1)
        # pdf_doc.line(x, y, x + self.width, y)

    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_line(container_offset_x, container_offset_y, self)
...
class TextElement(DocElement):
    ...
    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_text(container_offset_x, container_offset_y, self)
...
class TextBlockElement(DocElementBase):
    ...
    def render_pdf(self, container_offset_x, container_offset_y, pdf_doc):
        ...
        if not self.style.background_color.transparent:
            ...
            if self.style.ellipse_value:
                pdf_doc.ellipse(self.x + container_offset_x, y, self.width, self.height, style='F')
            else:
...
class TableElement(DocElement):
    ...
    def render_sbpl(self, container_offset_x, container_offset_y, renderer):
        renderer.render_table(container_offset_x, container_offset_y, self)
