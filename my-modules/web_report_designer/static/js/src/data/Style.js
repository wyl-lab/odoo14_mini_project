...
export default class Style {
    constructor(id, initialData, rb) {
        ...
        this.font = Style.font.yahei;
        this.fontSize = 12;
        this.lineSpacing = 1;
        this.borderColor = '#000000';
        this.borderWidth = '1';
        this.ellipseValue = false;
    ...
    getFields() {
        return ['id', 'name', 'bold', 'italic', 'underline', 'strikethrough',
            ...
            'ellipseValue','borderAll', 'borderLeft', 'borderTop', 'borderRight', 'borderBottom',
    ...
    setValue(field, value, elSelector, isShown) {
        this[field] = value;
        if (field.indexOf('border') !== -1 || field.indexOf('ellipseValue') !== -1) {
    ...
    setBordersAll(fieldPrefix, value) {
        this[fieldPrefix + 'borderAll'] = value;
        this[fieldPrefix + 'borderLeft'] = value;
        this[fieldPrefix + 'borderTop'] = value;
        this[fieldPrefix + 'borderRight'] = value;
        this[fieldPrefix + 'borderBottom'] = value;
    }
    ...
    static setBorderValue(obj, field, fieldPrefix, value, elSelector, isShown) {
        if (field === `${fieldPrefix}borderAll`) {
            ...
            if (obj.getValue(`${fieldPrefix}borderLeft`) && obj.getValue(`${fieldPrefix}borderTop`) &&
                ...
                if (obj.ellipseValue){
                    obj.setBordersAll(fieldPrefix, false);
                    if (isShown) {
                        $(elSelector).parent().find(`button[value="${fieldPrefix}borderAll"]`).removeClass('rbroButtonActive');
                        $(elSelector).parent().find(`button[value="${fieldPrefix}borderLeft"]`).removeClass('rbroButtonActive');
                        $(elSelector).parent().find(`button[value="${fieldPrefix}borderTop"]`).removeClass('rbroButtonActive');
                        $(elSelector).parent().find(`button[value="${fieldPrefix}borderRight"]`).removeClass('rbroButtonActive');
                        $(elSelector).parent().find(`button[value="${fieldPrefix}borderBottom"]`).removeClass('rbroButtonActive');
                    }
                } else {
        ...
        } else if (field === `${fieldPrefix}ellipseValue`) {
            if (obj.borderLeft || obj.borderTop || obj.borderRight || obj.borderBottom){
                obj.setBordersAll(fieldPrefix, true);
                if (isShown) {
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderAll"]`).addClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderLeft"]`).addClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderTop"]`).addClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderRight"]`).addClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderBottom"]`).addClass('rbroButtonActive');
                }
            } else {
                obj.setBordersAll(fieldPrefix, false);
                if (isShown) {
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderAll"]`).removeClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderLeft"]`).removeClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderTop"]`).removeClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderRight"]`).removeClass('rbroButtonActive');
                    $(elSelector).parent().parent().parent().find(`button[value="${fieldPrefix}borderBottom"]`).removeClass('rbroButtonActive');
                }
            }
        }
    }
