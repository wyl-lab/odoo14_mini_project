...
export default class MenuPanel {
    ...
    render() {
        ...
        elElementsDiv.append($(`<div id="rbro_menu_element_qr_code" class="rbroButton rbroMenuButton" draggable="true"
                title="${this.rb.getLabel('docElementQrCode')}">
                    <span class="rbroIcon-qrcode"></span>
                </div>`)
            .on('dragstart', event => {
                event.originalEvent.dataTransfer.setData('text/plain', '');
                event.originalEvent.dataTransfer.effectAllowed = 'copy';
                this.rb.startBrowserDrag('docElement', DocElement.type.qrCode, '');
                event.stopPropagation();
            })
            .on('touchstart', event => {
                this.rb.startBrowserDrag('docElement', DocElement.type.qrCode, '');
