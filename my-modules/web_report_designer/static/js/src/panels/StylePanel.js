...
export default class StylePanel {
    ...
    static renderStyle(elPanel, idPrefix, fieldPrefix, elementType, panel, rb) {
        ...
        if (elementType === DocElement.type.none || elementType === DocElement.type.text) {
            ...
            elDiv = $('<div class="rbroFormRow"></div>');
            elDiv.append(`<label for="rbro_${idPrefix}ellipse_value">${rb.getLabel('styleEllipseValue')}:</label>`);
            elFormField = $('<div class="rbroFormField"></div>');
            let elEllipseValue = $(`<input id="rbro_${idPrefix}ellipse_value" type="checkbox">`)
                .change(event => {
                    if (rb.getDataObject(panel.getSelectedObjId()) !== null) {
                        let cmd = new SetValueCmd(panel.getSelectedObjId(),
                            `rbro_${idPrefix}ellipse_value`, `${fieldPrefix}ellipseValue`,
                            elEllipseValue.is(":checked"), SetValueCmd.type.checkbox, rb);
                        rb.executeCommand(cmd);
                    }
                });
            elFormField.append(elEllipseValue);
            elDiv.append(elFormField);
            elPanel.append(elDiv);
    ...
    static updateStyleData(data, idPrefix, fieldPrefix, elementType) {
        if (data !== null) {
            ...
            $(`#rbro_${idPrefix}ellipse_value`).prop('disabled', false);
            $(`#rbro_${idPrefix}background_color`).spectrum('enable');
            $(`#rbro_${idPrefix}ellipse_value`).prop('checked', false);
            ...
            if (elementType === DocElement.type.none || elementType === DocElement.type.text || elementType === DocElement.type.image) {
                ...
                $(`#rbro_${idPrefix}ellipse_value`).prop('checked', data.getValue(`${fieldPrefix}ellipseValue`));
            ...
            if (elementType === DocElement.type.none || elementType === DocElement.type.text || elementType === DocElement.type.image) {
                $(`#rbro_${idPrefix}ellipse_value`).prop('disabled', true);
